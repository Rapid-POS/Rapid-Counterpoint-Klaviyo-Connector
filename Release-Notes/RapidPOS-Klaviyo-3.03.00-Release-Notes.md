...
### Updated Functionality for _Run Klaviyo Connector_ (Manually) via Menu Button 
...
- Updated the order of operations for **Customer Down** updates. 
  - Email and SMS Phone values are now applied to the Counterpoint customer record first (`AR_CUST`). 
  - After the Counterpoint update succeeds, the corresponding Klaviyo customer record (`USER_KLAVIYO_CUST`) is updated to reflect the new values. 
...
### Updated Install Script for _Klaviyo Custom Properties Documents Up_ Table 
...
- Fixed an issue where the process ID (PID) was not retained during initial Klaviyo customer uploads (during new installs). 
...
### Refined _Retain Counterpoint Value if Klaviyo is Empty_ Logic for Calculated Fields 
...
- Refined logic for calculated customer fields when **Retain Counterpoint Value if Klaviyo is Empty** is enabled. 
  ...
  - This change applies to **Klaviyo Field Mapping Customers Down**. 
